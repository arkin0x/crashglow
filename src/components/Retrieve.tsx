    ...
    const LoadedGame = () => {
        ...
        for (const [key, value] of Object.entries(assets)) {
            if (key.startsWith("text/html")) {
                return <BitsyGame game={value} setPlaying={setPlaying} />;
            }
            if (key.startsWith("text/javascript")) {
                return <Pico8Game key={id} gameJS={value} setPlaying={setPlaying} />;
            }
        }
        return null;
    };
    ...
    return (
        <div id="component-retrieve" className="primary">
            {notFound ? (
                ...
                <div className="layout">
                    <h2>Load a Game</h2>
                    <label htmlFor="nevent">Enter a game's nevent, event id, or UUID:</label>
                    <br />
                    <br />
                    <input
                        ref={identifierRef}
                    ...
                    <br />
                    <br />
                    <button className="button" disabled={gettingGame || !ndk} onClick={getGame}>
                        Play 🕹️
                    </button>
                </div>
            )}
